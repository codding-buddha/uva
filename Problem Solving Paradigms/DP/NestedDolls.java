...
class NestedDolls {
	public static void main(String[] args) {
		...
		while(tc-- > 0) {
			int m = in.nextInt();
			Dimension[] dolls = new Dimension[m];
			//List<List<Dimension>> lookup = new ArrayList<List<Dimension>>();
			List<Dimension> lookup = new ArrayList<Dimension>();
			for(int i = 0; i < m; i++) {
				dolls[i] = new Dimension(in.nextInt(), in.nextInt());
			}
			Arrays.sort(dolls);
			lookup.add(dolls[0]);
			for(int i = 1; i < dolls.length; i++) {
				int indx = search(lookup, dolls[i].w);
				if(lookup.get(indx).w > dolls[i].w) {
					lookup.get(indx).w = dolls[i].w;
				} else {
					lookup.add(dolls[i]);
				}
			}

			out.println(lookup.size());
		}
	...
	static int search(List<Dimension> lst, int w) {
		int l = 0, h = lst.size() - 1;
		while(l < h) {
			int m = l + (h-l)/2;
			Dimension v = lst.get(m);
			if(v.w <= w) {
				l = m+1;
			}else{
				h = m;
	...
	static class Dimension implements Comparable<Dimension> {
		...
		public int compareTo(Dimension d) {
			return this.h == d.h ? this.w - d.w : d.h - this.h;
		}
	}
